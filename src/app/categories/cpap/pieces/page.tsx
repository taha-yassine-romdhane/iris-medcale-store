...
export default function CpapPartsPage() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        // Fetch only CPAP parts
        const response = await fetch('/api/products?category=cpap&type=Pièce');
        ...
        <div className="relative max-w-screen-xl mx-auto px-6 sm:px-8 lg:px-12 py-20">
          <div className="text-center">
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold mb-6 text-transparent bg-clip-text bg-gradient-to-r from-white to-blue-300">
              Pièces Détachées CPAP
            </h1>
            <p className="text-lg sm:text-xl lg:text-2xl text-blue-100 max-w-3xl mx-auto leading-relaxed">
              Découvrez notre sélection de pièces détachées pour machines CPAP.
              Des composants essentiels pour maintenir la performance de votre appareil.
            </p>
          </div>
        </div>
